...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "SELECT projects.title AS project_title, 
   SUM(pledges.amount) - projects.funding_goal AS amount_over_goal FROM projects
   INNER JOIN pledges
   ON projects.id = pledges.project_id
...
def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_summed_amount
  "SELECT users.name, SUM(pledges.amount) AS pledges_sum FROM users
  JOIN pledges ON users.id = pledges.user_id
  GROUP BY users.name ORDER BY pledges_sum;"
end
